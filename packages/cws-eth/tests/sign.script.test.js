...
test('should call new generation functions ', async () => {
  expect(coreModule.core.flow.prepareSEData).toHaveBeenCalledTimes(0);
  expect(coreModule.core.flow.sendDataToCoolWallet).toHaveBeenCalledTimes(0);
  expect(coreModule.core.flow.sendScriptAndDataToCard).toHaveBeenCalledTimes(1);
});
