...
export default class XTZ extends COIN.EDDSACoin implements COIN.Coin {
  ...
  async signTransaction(signTxData: types.SignTxData, operation: types.xtzTransaction) {
    ...
    const signature = await xtzSign.signTransaction(signTxData, script, argument);
    const formatTxData = await xtzUtil.getFormatTransfer(operation);
    return txUtil.getSubmitTransaction(formatTxData, signature);
  }

  async signReveal(signTxData: types.SignTxData, operation: types.xtzReveal) {
    const { addressIndex } = signTxData;
  ...
  // TBD
  async signOrigination(signTxData: types.SignTxData, operation: types.xtzOrigination) {
    const { addressIndex } = signTxData;
  ...
  async signDelegation(signTxData: types.SignTxData, operation: types.xtzDelegation) {
    const { addressIndex } = signTxData;
  ...
  async signUndelegation(signTxData: types.SignTxData, operation: types.xtzDelegation) {
    const { addressIndex } = signTxData;
  ...
  async signSmartContract(signTxData: types.SignTxData, operation: types.xtzSmart) {
    const { addressIndex } = signTxData;
  ...
  async signTokenTransfer(signTxData: types.SignTxData, operation: types.xtzToken) {
    const { addressIndex } = signTxData;
    ...
    const argument = await argUtil.getTokenArgument(
      this.pathStyle,
      operation,
      addressIndex,
      parameters,
