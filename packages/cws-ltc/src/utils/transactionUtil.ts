...
export function addressToOutScript(address: string): ({ scriptType: types.ScriptType, outScript: Buffer, outHash?: Buffer }) {
...
export function pubkeyToAddressAndOutScript(burPublicKey: Buffer, scriptType: types.ScriptType)
...
export function createUnsignedTransactions(
  ...
  const outputArray = [
    Buffer.concat([bufferUtil.toUintBuffer(output.value, 8), outputScriptLen, outputScript])
  ];
