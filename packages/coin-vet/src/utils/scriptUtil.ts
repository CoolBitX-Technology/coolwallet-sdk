...
const getTransferArgument = (transaction: types.Record) => {
  ...
  const chainTag = stringUtil.handleHex(transaction.chainTag.toString()).padStart(1, '0');
  ...
  const expiration = stringUtil.handleHex(transaction.expiration.toString()).padStart(4, '0');
  ...
  const value = stringUtil.handleHex(clause.value.toString()).padStart(32, '0');
  console.log(`value: ${clause.value} hex: ${value}`);

  const data = stringUtil.handleHex(clause.data);
  console.log(`data: ${clause.data} hex: ${data}`);

  const gasPriceCoef = stringUtil.handleHex(transaction.gasPriceCoef.toString()).padStart(1, '0');
  console.log(`gasPriceCoef: ${transaction.gasPriceCoef} hex: ${gasPriceCoef}`);

  const gas = stringUtil.handleHex(transaction.gas.toString()).padStart(8, '0');
  console.log(`gas: ${transaction.gas} hex: ${gas}`);

  let dependsOn: string;
  if (transaction.dependsOn == null) {
    dependsOn = ''.padStart(32, '0');
  } else {
    dependsOn = stringUtil.handleHex(transaction.dependsOn).padStart(32, '0');
  }
  console.log(`dependsOn: ${transaction.dependsOn} dependsOn: ${dependsOn}`);

  const nonce = stringUtil.handleHex(transaction.nonce.toString()).padStart(8, '0');
...
export const getScriptAndArguments2 = async (addressIndex: number, transaction: types.Record) => {
  ...
  const script =
    '03040E01C7070000000332C2A0D700FFFFCAA1C70008C2ACD70009FFFCA00700CAAC27000DC2ACD70021FFE0CAAC970041BE0700C2ADD7002750FFFFC2ADD7002751FFF8CAAD57002759C2ADD7002779FFF8DC07C003564554DAACD7B021FFE0D207CC05065052455353425554546F4E' +
