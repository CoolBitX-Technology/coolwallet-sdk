...
import * as polyUtil from './utils/polygonUtils';
import * as polyUtilEIP1559 from './utils/polygonUtilsEIP1559';
...
export const signEIP1559Transaction = async (
  ...
  const rawPayload = polyUtilEIP1559.getRawHex(transaction);
  ...
  if (!Buffer.isBuffer(canonicalSignature)) {
    const { v, r, s } = await polyUtilEIP1559.genEthSigFromSESig(canonicalSignature, rlp.encode(rawPayload), publicKey);
    const serializedTx = polyUtilEIP1559.composeSignedTransacton(rawPayload, v, r, s);
    return serializedTx;
  } else {
...
export const signEIP1559SmartContractTransaction = async (
  ...
  if (!Buffer.isBuffer(canonicalSignature)) {
    const { v, r, s } = await polyUtilEIP1559.genEthSigFromSESig(canonicalSignature, rlp.encode(rawPayload), publicKey);
    return polyUtilEIP1559.composeSignedTransacton(rawPayload, v, r, s);
  } else {
    throw new error.SDKError(signEIP1559Transaction.name, 'canonicalSignature type error');
  }
};
...
export const signTransaction = async (
  ...
  const rawPayload = polyUtil.getRawHex(transaction);
...
/**
 * sign Polygon Smart Contract Transaction
 * @param {Transport} transport
 * @param {string} appId
 * @param {String} appPrivateKey
 * @param {coinType} coinType
