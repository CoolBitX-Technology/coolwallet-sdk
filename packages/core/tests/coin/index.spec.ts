...
import { signECDSA } from '../../src/coin/sign';
...
describe('Test signECDSA', () => {
  ...
  it('should sign eth transaction hash correctly', async () => {
    ...
    const hashData = Buffer.from(keccak_256(Buffer.from(rlpEncodedRawData, 'hex'))).toString('hex');
    const addressIndex = 0;
    const coinType = '8000003c';
    const chainId = 1;
    const canonicalSignature = (await signECDSA({
      transport,
      coinType,
      addressIndex,
      depth: 5,
    ...
    const expected = Buffer.from((await node.sign(hashData)) ?? '').toString('hex');
    ...
    const recoveryParam = ecdsa.getRecoveryParam(hashData, canonicalSignature, publicKey);
