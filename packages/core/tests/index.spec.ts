import { Transport, coin, config, apdu, tx } from '../src';
...
describe('Test CoolWallet SDK Core Functional', () => {
  ...
  it('Test rlp erc20 execute script', async () => {
    ...
    const sent = () => apdu.tx.sendScript(transport, script + `FA`.padEnd(144, '0'));
    ...
    const se_signature = (await tx.flow.getSingleSignatureFromCoolWallet(
      transport,
      [sent],
      executeRlpScript,
      false,
  ...
  it('Test algorand', async () => {
    ...
    const executeRlpScript = () =>
      apdu.tx.executeRlpScript(transport, props.appId, props.appPrivateKey, path, [
