...
async function getSEPublicKey(transport: Transport): Promise<string> {
  ...
  const compressedPublicKey = getCompressedPublicKey(SE_KEY_PARAM[transport.cardType].chipMasterPublicKey);

  const addend = sha512(
    Buffer.from(SE_KEY_PARAM[transport.cardType].chipMasterChainCode, 'hex'),
  ...
  const chipMasterPublicKey = secp256k1
    .keyFromPublic(Buffer.from(SE_KEY_PARAM[transport.cardType].chipMasterPublicKey, 'hex'))
    .getPublic();
