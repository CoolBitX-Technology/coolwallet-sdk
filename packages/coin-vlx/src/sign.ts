...
import * as vlxUtil from './utils/vlxUtils';
import * as vlxUtilEIP1559 from './utils/vlxUtilsEIP1559';
...
export const signEIP1559Transaction = async (
  ...
  const rawPayload = vlxUtilEIP1559.getRawHex(transaction);
  ...
  if (!Buffer.isBuffer(canonicalSignature)) {
    const { v, r, s } = await vlxUtilEIP1559.genVlxSigFromSESig(canonicalSignature, rlp.encode(rawPayload), publicKey);
    const serializedTx = vlxUtilEIP1559.composeSignedTransacton(rawPayload, v, r, s);
    return serializedTx;
  } else {
...
export const signEIP1559SmartContractTransaction = async (
  ...
  if (!Buffer.isBuffer(canonicalSignature)) {
    const { v, r, s } = await vlxUtilEIP1559.genVlxSigFromSESig(canonicalSignature, rlp.encode(rawPayload), publicKey);
    return vlxUtilEIP1559.composeSignedTransacton(rawPayload, v, r, s);
  } else {
    throw new error.SDKError(signEIP1559Transaction.name, 'canonicalSignature type error');
  }
};
...
export const signTransaction = async (
  ...
  const rawPayload = vlxUtil.getRawHex(transaction);
  ...
  if (!Buffer.isBuffer(canonicalSignature)) {
    const { v, r, s } = await vlxUtil.genVlxSigFromSESig(canonicalSignature, rlp.encode(rawPayload), publicKey);
    return vlxUtil.composeSignedTransacton(rawPayload, v, r, s, transaction.chainId);
  } else {
...
/**
 * sign vlx Smart Contract Transaction
 * @param {Transport} transport
 * @param {string} appId
 * @param {String} appPrivateKey
 * @param {coinType} coinType
...
export const signSmartContractTransaction = async (
  ...
  if (!Buffer.isBuffer(canonicalSignature)) {
    const { v, r, s } = await vlxUtil.genVlxSigFromSESig(canonicalSignature, rlp.encode(rawPayload), publicKey);
    const serializedTx = vlxUtil.composeSignedTransacton(rawPayload, v, r, s, transaction.chainId);
    return serializedTx;
  } else {
    throw new error.SDKError(signTransaction.name, 'canonicalSignature type error');
  }
...
export const signMessage = async (
  ...
  const prefix = 'vlxereum Signed Message:';
  ...
  if (!Buffer.isBuffer(canonicalSignature)) {
    const { v, r, s } = await vlxUtil.genVlxSigFromSESig(canonicalSignature, payload, publicKey);
