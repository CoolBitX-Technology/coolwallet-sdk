...
import * as vlxSign from './sign';
import { pubKeyToAddress } from './utils/vlxUtils';
...
export default class VLX extends COIN.ECDSACoin implements COIN.Coin {
  constructor() {
  ...
  async signEIP1559Transfer(signTxData: types.signEIP1559Tx): Promise<string> {
    ...
    return vlxSign.signEIP1559Transaction(signTxData, script, argument, publicKey);
  }
  ...
  async signEIP1559Smart(signTxData: types.signEIP1559Tx): Promise<string> {
    ...
    if (signTxData.transaction.data.length > 8000) {
      ...
      return vlxSign.signEIP1559SmartContractTransaction(signTxData, script, argument, publicKey);
    }
  ...
  async signTransferTransaction(signTxData: types.signTx): Promise<string> {
    ...
    return vlxSign.signTransaction(signTxData, script, argument, publicKey);
  }
  ...
  async signSmartContractTransaction(signTxData: types.signTx): Promise<string> {
    ...
    if (signTxData.transaction.data.length > 8000) {
      ...
      return vlxSign.signSmartContractTransaction(signTxData, script, argument, publicKey);
    }
  ...
  async signMessage(signMsgData: types.signMsg): Promise<string> {
    ...
    return vlxSign.signMessage(signMsgData, script, argument, publicKey);
  }
  ...
  async signTypedData(typedData: types.signTyped): Promise<string> {
    ...
    return vlxSign.signTypedData(typedData, script, publicKey);
  }
}
