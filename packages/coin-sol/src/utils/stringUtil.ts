...
import { TX_TYPE } from '../config/params';
...
export const getTxType = (transaction: TransactionType): string => {
  if (transaction.options) {
    ...
    if (isFieldsValid([programId, data, owner])) return TX_TYPE.SMART_CONTRACT;
    else if (isFieldsValid([programId, owner, decimals, value])) return TX_TYPE.SPL_TOKEN;
  }
